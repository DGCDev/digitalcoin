...
void AddTimeData(const CNetAddr& ip, int64 nTime)
{
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
		...
		if (nTime > 2000000000) //SWITCHOVER_TIME
			max_adjust = 10;
		else
			max_adjust = 35;
		
        if (abs64(nMedian) < max_adjust * 60) // DigitalCoin: changed maximum adjust to 10 mins to avoid letting peers change our time too much in case of an attack.
        {
            nTimeOffset = nMedian;
        }
        else
