...
{
    // DigitalCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
	...
	if (pblock->nTime > SWITCHOVER_TIME)
		{
			static const int64	BlocksTargetSpacing			= 40.0;							// 40 seconds
			unsigned int		TimeDaySeconds				= 60 * 60 * 24;
			int64				PastSecondsMin				= TimeDaySeconds * (2.0/3.0) * 0.1;
			int64				PastSecondsMax				= TimeDaySeconds * (2.0/3.0) * 2.8;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
	...
	// DigitalCoin: Special short-term limits to avoid 10,000 BDB lock limit:
    if (GetBlockTime() < SWITCHOVER_TIME) // Currently: Always enforced
    {
    ...
    // Check timestamp
	if (GetBlockTime() >= SWITCHOVER_TIME && GetBlockTime() > GetAdjustedTime() + 30 * 60)
		return state.Invalid(error("CheckBlock() : block timestamp too far in the future"));
    else if (GetBlockTime() < SWITCHOVER_TIME && GetBlockTime() > GetAdjustedTime() + 2 * 60 * 60)
        return state.Invalid(error("CheckBlock() : block timestamp too far in the future"));
...
{
    // DigitalCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
void static DigitalCoinMiner(CWallet *pwallet)
{
    printf("DigitalCoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running DigitalCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DigitalCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DigitalCoinMiner, pwallet));
}
