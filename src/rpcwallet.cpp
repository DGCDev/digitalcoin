...
Value sendopreturn(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 3)
        throw runtime_error(
            "sendopreturn \"digitalcoinaddress\" amount \"data\"\n"
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress \"digitalcoinaddress\" ( minconf )\n"
            "\nReturns the total amount received by the given digitalcoinaddress in transactions with at least minconf confirmations.\n"
            "\nArguments:\n"
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            "sendfrom \"fromaccount\" \"todigitalcoinaddress\" amount ( minconf \"comment\" \"comment-to\" )\n"
            "\nSent an amount from an account to a digitalcoin address.\n"
...
Value sendmany(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric) The digitalcoin address is the key, the numeric amount in btc is the value\n"
            "      ,...\n"
            "    }\n"
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; Digitalcoin server stopping, restart to run with encrypted wallet. The keypool has been flushed, you need to make a new backup.";
}
